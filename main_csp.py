...
class CSP_Model:
	...
	def __init__(self):
		...
		self.data_path 	= 'dataset/'
		 ...
		 #iloc not being read
		#X= self.data_path.iloc[:, 0:4].values
		#y= self.data_path.iloc[:, 4].values
	...
	def run_csp(self):
		...
		#clf.fit(feature_mat,self.train_label) #comment out?
		...
		success_rate = lda.score(eval_feature_mat,self.eval_label)
